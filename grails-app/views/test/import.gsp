                    ...
                    <div class="col-md-12 col-lg-10 col-lg-offset-1">
                        ...
                        <div class="portlet light bg-inverse logConfig-portlet">
                            <div class="portlet-title">
                                ...
                                <div>
                                    <a href="${resource(dir: 'files', file: 'STT_TestTemplate.csv')}" download="" class="btn green-dark button-template icon-button-container"><i class="fa fa-download icon-button" aria-hidden="true"></i><g:message code="default.import.template" default="Template"/></a>
                                </div>
                            </div>
                                ...
                                <div class="scroller" style="height:540px" data-rail-visible="1" data-rail-color="#105d41" data-handle-color="#4A9F60">
                                    ...
                                    <p>
                                        ${raw(g.message(code: 'default.import.description.test', default: 'To import correctly the test, you must follow the following scheme: <strong>| Name<span style="color: #D05454">*</span> | Description<span style="color: #D05454">*</span> |' +
                                                ' Active<span style="color: #D05454">*</span> | Number of questions<span style="color: #D05454">*</span> | Initial date<span style="color: #D05454">*</span> | End date<span style="color: #D05454">*</span> | Maximum time<span style="color: #D05454">*</span> | ' +
                                                'Do incorrect answers discount?<span style="color: #D05454">*</span> | Maximum number of attempts<span style="color: #D05454">*</span> | Penalty for each extra attempt<span style="color: #D05454">*</span> | Topic<span style="color: #D05454">*</span> |' +
                                                ' Catalog<span style="color: #D05454">*</span> | </strong>; where name must be unique, ie, be available. <br><br> Fields marked with <span style="color: #D05454">*</span> are required, the rest are optional being mandatory that the column is' +
                                                ' in the <strong>.csv</strong> document although the corresponding fields in each row are empty.'))}
                                    </p>
                                    <p>
                                        ${raw(g.message(code: 'default.import.description.test.recommendation', default: '<i>Active</i> and <i>Do incorrect answers discount?</i> fields can have two values (<strong>true</strong> and <strong>false</strong>) depending on whether the ' +
                                                'test is active or not and if the score of the user is penalized (1/3 of the average score of all questions for each incorrect answer) for incorrect answers, respectively.'))}
                                    </p>
                                    <p>
                                        ${raw(g.message(code: 'default.import.description.test.remember', default: 'Besides, <strong>remember:</strong><ul><li>The number of question must be <strong>less or equal</strong> to number of question that the catalog selected contains.</li>' +
                                                '<li>Initial date of the test must have the following format <strong>dd-MM-yyyy</strong>; where <i>dd</i>: day, <i>MM</i>: month and <i>yyyy</i>: year in numeric format. <strong>Important!</strong> You must configure the date format in ' +
                                                'the sotfware used. Example: The exact path depends on the software used. Approximate path: <i>Configuration/Preferences -> Format -> Cells -> Date</i>.</li><li>End date of the test must have the following format <strong>dd-MM-yyyy</strong>; ' +
                                                'where <i>dd</i>: day, <i>MM</i>: month and <i>yyyy</i>: year in numeric format. In addition, it can not be earlier than the initial date.</li><li><i>Maximum time</i> field must be expressed in <strong>minutes</strong>.</li>' +
                                                '<li><i>Maximum number of attempts</i> field must have a value between <strong>1</strong> and <strong>5</strong>.</li><li><i>Penalty for each extra attempt</i> must have a value between <strong>0%</strong> and <strong>100%</strong> ' +
                                                'where it is only valid when the maximum number of allowed attempts is greater than 1.</li><li>In the <i>Topic</i> field, you must type <strong>only the name</strong> of the topic relationed to test. <strong>It is important, the prior existence ' +
                                                'in the system of the topic indicated.</strong></li></ul><li>In the <i>Catalog</i> field, you must type <strong>only the name</strong> of the catalog relationed to test. <strong>It is important, the prior existence in the system of the catalog ' +
                                                'indicated.</strong></li></ul>'))}
                                    </p>
                                </div>
                            </div>
                        </div> <!-- /. Portlet -->
                            ...
                            <div class="fileinput fileinput-new fileinput-import" data-provides="fileinput">
                                <div class="input-group btn-block input-import">
                                    ...
                                    <span class="input-group-addon btn btn-block grey-gallery btn-file">
                                        ...
                                        <input type="file" accept="text/comma-separated-values, text/csv, application/csv, application/excel, application/vnd.ms-excel, application/vnd.msexcel, text/anytext" name="importFileTest" id="importFileTest" required />
                                    </span>
